    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => addTag("suitableFor", "suitableInput")} className="w-9 h-9 bg-teal-600 text-white rounded-xl flex items-center justify-center hover:bg-teal-700 transition-colors flex-shrink-0">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => addTag("examples", "exampleInput")} className="w-9 h-9 bg-teal-600 text-white rounded-xl flex items-center justify-center hover:bg-teal-700 transition-colors flex-shrink-0">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
              </button>
            </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setForm((f) => ({ ...f, active: !f.active }))} className={`relative w-11 h-6 rounded-full transition-colors ${form.active ? "bg-teal-600" : "bg-gray-300"}`}>
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <button onClick={() => { if (form.name && form.description) { onSave(form); onClose(); } }} className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-teal-600 hover:bg-teal-700 text-white transition-colors">
    ...
    <div className="bg-gray-50 font-sans">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Vehicle Types (Passenger Vehicles)</h1>
            <p className="text-sm text-gray-500 mt-0.5">Manage accepted vehicle categories from passengers</p>
          </div>
          <button onClick={() => { setEditing(null); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium bg-teal-600 hover:bg-teal-700 text-white transition-colors flex-shrink-0">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            Add Vehicle Type
          </button>
        </div>
        ...
        <div className="bg-teal-50 border border-teal-100 rounded-2xl p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-teal-800">About Vehicle Types</p>
            <p className="text-sm text-teal-700 mt-0.5">These categories define what types of passenger-owned vehicles are accepted on the Feur platform. Passengers register their own vehicles, and professional drivers are hired to drive them.</p>
          </div>
        </div>
            ...
            <div key={vt.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button onClick={() => toggleActive(vt.id)} className={`px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors ${vt.active ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {vt.suitableFor.map((s) => (
                        <span key={s} className="px-2.5 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-lg font-medium">{s}</span>
                      ))}
                    </div>
                  </div>
