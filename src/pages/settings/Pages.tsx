    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <button onClick={() => { if (form.title && form.slug && form.content) { onSave(form); onClose(); } }} className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-teal-600 hover:bg-teal-700 text-white transition-colors">{page ? "Save Changes" : "Create Page"}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-gray-50 font-sans">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          <button onClick={() => { setEditingPage(null); setShowCreate(true); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium bg-teal-600 hover:bg-teal-700 text-white transition-colors flex-shrink-0">
        ...
        <div className="flex gap-2 mb-5">
          {(["all", "published", "draft"] as const).map((f) => (
            <button key={f} onClick={() => setFilterStatus(f)} className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-colors ${filterStatus === f ? "bg-teal-600 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>{f}</button>
          ))}
        </div>
          ...
          {filtered.map((pg) => (
            <div key={pg.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="flex border-t border-gray-100">
                ...
                <button onClick={() => setPreviewPage(pg)} className="px-5 py-3 text-gray-400 hover:text-teal-600 hover:bg-teal-50 transition-colors">
